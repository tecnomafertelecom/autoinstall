...
$(document).ready(function () {
  ...
  $("#issabel-callcenter-cejillas-contenido").tabs({
    ...
    add: function (event, ui) {
      if (externalurl != null)
        $(ui.panel).append(
          '<iframe scrolling="auto" height="450" frameborder="0" width="100%" src="' +
            externalurl +
            '" />'
        );
      externalurl = null;
  ...
  for (var i = 0; i < t_curr_uiversion.length; i++)
    curr_uiversion[i] = parseInt(t_curr_uiversion[i]);
  ...
  if ($("#issabel-callcenter-llamada-paneles").length > 0) {
    $("#issabel-callcenter-llamada-paneles").layout({
      fxName: "none",
      west: { size: 300 },
    });
    $("#issabel-callcenter-llamada-paneles-izq").layout({
      fxName: "none",
      south: { size: 250 },
    });
  }
...
function initialize_client_state(nuevoEstado) {
  ...
  iniciar_cronometro(
    nuevoEstado.timer_seconds !== "" ? nuevoEstado.timer_seconds : null
  );
...
function actualizar_cronometro() {
  ...
  $("#issabel-callcenter-cronometro").text(
    tiempo[2] + ":" + tiempo[1] + ":" + tiempo[0]
  );
...
function apply_ui_styles(uidata) {
  ...
  schedule_call_error_msg_missing_date =
    uidata.schedule_call_error_msg_missing_date;
...
function do_login() {
  $.post(
    "index.php?menu=" + module_name + "&rawmode=yes",
    {
      menu: module_name,
      rawmode: "yes",
      action: "doLogin",
  ...
  $.post(
    ...
    function (respuesta) {
      ...
      if (respuesta["action"] == "login") {
        ...
        console.log("INICIANDO SOCKET:", hostname);
        const socket = io.connect(`https://${hostname}:3333`); // Substitua pela URL do seu servidor Socket.IO
...
function login_estado_espera(msg) {
  $("#login_icono_espera").attr(
    "style",
    "visibility: visible; position: none;"
  );
...
function do_schedule() {
  ...
  if (
    $("#schedule_type_bydate").is(":checked") &&
    ($("#schedule_date_start").datepicker("getDate") == null ||
      $("#schedule_date_end").datepicker("getDate") == null)
  ) {
    $("#issabel-callcenter-agendar-llamada-error-message-text").text(
      schedule_call_error_msg_missing_date
    );
    $("#issabel-callcenter-agendar-llamada-error-message").show(
      "slow",
      "linear",
      function () {
        setTimeout(function () {
  ...
  $.post(
    "index.php?menu=" + module_name + "&rawmode=yes",
    {
      ...
      data: {
        ...
        schedule_time_start:
          $("#schedule_time_start_hh").val() +
          ":" +
          $("#schedule_time_start_mm").val() +
          ":00",
        schedule_time_end:
          $("#schedule_time_end_hh").val() +
          ":" +
          $("#schedule_time_end_mm").val() +
          ":59",
      },
    },
    function (respuesta) {
...
function do_ping() {
  $.get(
    "index.php",
    { menu: module_name, action: "ping", rawmode: "yes" },
    function (respuesta) {
      verificar_error_session(respuesta);
  ...
  if (window.EventSource) {
    ...
    $.post(
      ...
      mostrar_mensaje_error(
        "Lost connection to server (Long-Polling), retrying..."
      );
...
function manejarRespuestaStatus(respuesta) {
  for (var i in respuesta) {
    if (respuesta[i].txt_estado_agente_inicial != null)
      $("#issabel-callcenter-estado-agente-texto").text(
        respuesta[i].txt_estado_agente_inicial
      );
    ...
    switch (respuesta[i].event) {
      ...
      case "agentlinked":
        ...
        if (respuesta[i].calltype == "incoming") {
          ...
          $("#llamada_saliente_nombres").text(
            respuesta[i].txt_contacto_nombres
          );
      ...
      case "iframe":
        if (jqueryui_tabs_use_refresh) {
          ...
          $("#issabel-callcenter-cejillas-contenido")
            .find(".ui-tabs-nav li.tab-externalurl")
            .remove();
          ...
          if (url != null) {
            $("#issabel-callcenter-cejillas-contenido").append(
              '<div id="tabs-externalurl"><iframe scrolling="auto" height="450" frameborder="0" width="100%" src="' +
                url +
                '" /></div>'
            );
            $(
              '<li class="tab-externalurl"><a href="#tabs-externalurl">' +
                externalurl_title +
                "</a></li>"
