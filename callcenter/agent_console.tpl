...
//SocketIO Events
            let elemento = document.getElementById("issabel-callcenter-titulo-consola");
            let infoAgent = elemento.innerText || elemento.textContent;
            let match = infoAgent.match(/SIP\/(\d+) - (.+)/);
            if (match) {
                var ramal = match[1];
                var nome = match[2];
                ...
                const socket = io.connect(`https://${hostname}:3333`);
                ...
                socket.on('agentsOnline', (data) =>{
                    const mapObject = new Map(Object.entries(JSON.parse(data)));
