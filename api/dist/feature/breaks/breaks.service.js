...
let BreaksService = class BreaksService {
    ...
    async create(data) {
        const existingBreak = await this.repository.findByReason(data.reason);
        if (existingBreak) {
            throw new Error('A pausa com a mesma razão já existe no banco de dados.');
        }
...
exports.BreaksService = BreaksService = __decorate([
    ...
    __metadata("design:paramtypes", [breaks_repository_1.BreaksRepository])
], BreaksService);
